...
async function fetchNewsArticles() {
  ...
  for (const query of SEARCH_QUERIES) {
    ...
    const url = `https://newsapi.org/v2/everything?q=${encoded}&from=${fromDate()}&sortBy=relevancy&language=en&pageSize=10&apiKey=${NEWS_API_KEY}`;
    try {
...
1. Select the 13 most important and interesting articles for our readers (scientists, investors, longevity enthusiasts)
2. For each selected article, write a punchy, dramatic, Drudge-style headline in ALL CAPS that conveys the key finding or development. Make them compelling. Max 120 characters.
3. Assign a category: "longevity", "clinical", or "science"
4. Return ONLY a JSON array, no other text, no markdown backticks.
...
${articleList}`;
  ...
  return ranked.map(item => {
    const original = candidates[item.originalIndex - 1] || {};
    return {
      title:       item.title,
